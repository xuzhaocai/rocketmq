...
public class MQClientAPIImpl {
    ...
    static {
        // 设置 remoting version ，其实就是设置当前的版本号
    ...
    public void updateNameServerAddressList(final String addrs) {
        String[] addrArray = addrs.split(";");// 使用分号分割多个addr
    ...
    public SendResult sendMessage(
        ...
        // sendSmartMsg 这个是默认开启的，这个东西算是一种优化吧
            ...
            new InvokeCallback() {
                ...
                public void operationComplete(ResponseFuture responseFuture) {
                    ...
                    if (null == sendCallback && response != null) {
                        ...
                        //更新容错选项   brokerName ： broker名字   currentLatency ：当前延迟，其实就是现在时间戳-开始发送的时间戳   isolation: 是否需要隔离
     ...
     * @param timeoutMillis  超时时间
     * @param request 请求command
     * @param sendCallback  用户自己的callback
     * @param topicPublishInfo topic信息
     * @param instance mq 客户端实例
     * @param timesTotal 重试次数
     * @param curTimes  当前第几次
     * @param e
     * @param context
     * @param needRetry  是否重试
     * @param producer
     */
    ...
    private SendResult processSendResponse(
        ...
        switch (response.getCode()) {
            ...
            case ResponseCode.SUCCESS: {//响应成功
                ...
                switch (response.getCode()) {
                    case ResponseCode.FLUSH_DISK_TIMEOUT://刷到磁盘超时
                        sendStatus = SendStatus.FLUSH_DISK_TIMEOUT;
                        break;
                    case ResponseCode.FLUSH_SLAVE_TIMEOUT:// 刷到从节点超时
                        sendStatus = SendStatus.FLUSH_SLAVE_TIMEOUT;
                        break;
                    case ResponseCode.SLAVE_NOT_AVAILABLE:// 从节点不可用
                ...
                if (msg instanceof MessageBatch) {// 一批消息，把每个消息唯一ID用逗号拼起来
                ...
                // TODO regionId 与 traceOn 不知道什么意思
                String regionId = response.getExtFields().get(MessageConst.PROPERTY_MSG_REGION);
                String traceOn = response.getExtFields().get(MessageConst.PROPERTY_TRACE_SWITCH);
                if (regionId == null || regionId.isEmpty()) {// 设置默认region id
    ...
    private PullResult processPullResponse(
        ...
        switch (response.getCode()) {
            case ResponseCode.SUCCESS:// 有新消息
                pullStatus = PullStatus.FOUND;
                break;
            case ResponseCode.PULL_NOT_FOUND:// 没有新消息
        ...
        return new PullResultExt(pullStatus,// 拉取状态
                                    responseHeader.getNextBeginOffset(),// 下一个开始的offset
                                    responseHeader.getMinOffset(),// 最大offset
                                    responseHeader.getMaxOffset(),// 最小offset
                        null,
                                    responseHeader.getSuggestWhichBrokerId(),// 建议下次拉取的brokerId
                                    response.getBody());// 内容
    }
    ...
    /**
     *  从broker获取某个topic 中某个mq 在某个时间的一个offset
     * @param addr  broker地址
     * @param topic  主题
     * @param queueId  mq id
     * @param timestamp 时间戳
    ...
    public long searchOffset(final String addr, final String topic, final int queueId, final long timestamp,
        ...
        // RemotingCommand   SEARCH_OFFSET_BY_TIMESTAMP   根据时间戳 获取 offset
    ...
    public List<String> getConsumerIdListByGroup(
        ...
        switch (response.getCode()) {
            ...
            case ResponseCode.SUCCESS: {// 成功返回的话
    ...
    public long queryConsumerOffset(
        ...
        ///QUERY_CONSUMER_OFFSET 请求消费offset
    ...
    public TopicRouteData getTopicRouteInfoFromNameServer(final String topic, final long timeoutMillis,
        ...
        assert response != null;
        switch (response.getCode()) {
            case ResponseCode.TOPIC_NOT_EXIST: {// topic 不存在
                // 允许不存在 && topic不是  TBW102
            ...
            case ResponseCode.SUCCESS: {// 成功
                byte[] body = response.getBody();// 获取内容
                if (body != null) {
